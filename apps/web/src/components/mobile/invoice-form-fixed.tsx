...
export default function MobileInvoiceFormFixed() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.business) {
            ...
            console.log('✅ Business settings loaded:', data.business)
            
            // Load FBR scenarios based on actual business type and sector
            const scenarios = getApplicableScenarios(
              data.business.businessType || 'Manufacturer', 
              data.business.sector || 'All Other Sectors'
            )
          ...
          if (response.status === 404) {
            console.warn('⚠️ Business not found, using default settings')
            setBusinessSettingsError('Business profile not found. Please complete your business settings.')
            // Use default scenarios
            const scenarios = getApplicableScenarios('Manufacturer', 'All Other Sectors')
            setAvailableScenarios(scenarios.scenarios)
        ...
        setBusinessSettingsError(error instanceof Error ? error.message : 'Failed to load business settings')
  ...
  const calculateTotals = useCallback(() => {
    ...
    try {
      if (selectedScenario && taxBreakdownData.length > 0) {
        const breakdown = taxCalculator.calculateTax(taxBreakdownData, businessSettings?.province || 'Punjab')
    ...
    // Mark form as dirty
    setIsDirty(true)
  }, [invoice.items, selectedScenario, taxCalculator, businessSettings])
  ...
  const handleProductSelect = useCallback((product: any, itemIndex: number) => {
    ...
    updateItem(itemIndex, 'price', product.unitPrice || product.price || 0)
    updateItem(itemIndex, 'hsCode', product.hsCode || '')
    updateItem(itemIndex, 'taxRate', product.taxRate || 18)
    updateItem(itemIndex, 'productId', product.id || '')
    
    console.log('🔍 MOBILE DEBUG: Price set to:', product.unitPrice || product.price || 0)
  ...
  const generateQRCode = useCallback(async () => {
    ...
    try {
      ...
      const businessNTN = businessSettings?.ntnNumber || (session?.user as any)?.businessNTN || 'DEFAULT_NTN'
  ...
  }, [invoice.invoiceNumber, invoice.invoiceDate, invoice.total, invoice.customerNTN, businessSettings, session?.user, showErrorToast])
  ...
  const saveInvoice = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      if (!invoiceNumber) {
        ...
        invoiceNumber = `${businessSettings?.invoicePrefix || 'INV-'}${year}${month}${day}-${random}`
      }
